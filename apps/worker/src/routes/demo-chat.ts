...
function buildCandidateJobNotificationFlex(job: DemoCompanyJob): Record<string, unknown> {
  return {
    ...
    body: {
      ...
      paddingAll: '16px',
      contents: [
        { type: 'text', text: job.title, size: 'md', color: SAIYO_PRO_BRAND_NAVY, weight: 'bold', wrap: true },
        { type: 'text', text: 'プロフィールと会話内容から、条件が近そうな求人として表示しています。', size: 'xs', color: SAIYO_PRO_BRAND_MUTED, wrap: true },
        { type: 'text', text: job.hourlyWage, size: 'sm', color: SAIYO_PRO_BRAND_MUTED, wrap: true },
        { type: 'text', text: job.shift, size: 'sm', color: SAIYO_PRO_BRAND_MUTED, wrap: true },
      ],
    },
    footer: {
      type: 'box',
      ...
      paddingAll: '12px',
      contents: [
        { type: 'button', style: 'primary', color: SAIYO_PRO_BRAND_PRIMARY, height: 'sm', action: { type: 'uri', label: '会社を見る', uri: `${DEMO_WORKER_URL}/demo-candidate-jobs?candidate=yamada&companyName=${encodeURIComponent(job.companyName)}` } },
      ],
    },
  };
}
